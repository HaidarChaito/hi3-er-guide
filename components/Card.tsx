      ...
      >
        <div className={`${styles.card} w-56 pb-1 sm:w-64`}>
          <div className='z-10 w-full text-white'>
            <Image objectFit='fill' className='h-48 w-full rounded-t-3xl	p-2' width={200} height={100} src={'/images/valks/' + valkery.label + '.png'} alt={valkery.label} />
        ...
        {isModalOpen && (
          <div className='fixed left-0	top-0 z-20 flex h-full w-full items-center justify-center overflow-auto backdrop-blur-sm backdrop-brightness-75 backdrop-saturate-50'>
            <dialog open id='myModal' className={`relative z-10 block h-4/5  w-4/5 overflow-scroll rounded-md border-2 bg-primary text-secondary opacity-90 border-${switchColor(valkery.type)}`}>
              <div className='relative z-30 flex flex-col'>
                <div className={`${styles.dialogBackground}`} style={{ backgroundImage: `url('/images/valks/${valkery.label}.png')` }} />
                <div className='my-2 flex h-auto w-full items-center justify-center'>
                  <div className='ml-10 flex h-auto w-full items-center justify-center text-lg font-bold sm:text-2xl'>{valkery.label}</div>
